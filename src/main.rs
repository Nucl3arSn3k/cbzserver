...
async fn library_send(data: web::Data<TreeState>) -> HttpResponse {
    ...
    let basedir = "C:\\Users\\User\\Documents\\GoldenAge"; //Swapping root dir
    let dbcon = dbconfig("cache.db".to_string());
    let mut vax = Vec::new();

    if dbcon == false {
        println!("Starting scan");
        vax = catalog_dir(Path::new("C:\\Users\\User\\Documents\\GoldenAge"), false).await; //Insert an arbiter here?
